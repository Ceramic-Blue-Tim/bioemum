...
AxiLite::AxiLite(uint64_t offset, uint32_t range, uint16_t nb_regs_write, uint16_t nb_regs_read){
    ...
    if(fd != -1){
        ...
        _axi_regs_u32_vptr = (uint32_t *)mmap(NULL, range, PROT_READ|PROT_WRITE, MAP_SHARED, fd, offset);
 ...
 * @return None
****************************************************************************/
AxiLite::AxiLite(uint64_t offset, uint16_t nb_regs_write, uint16_t nb_regs_read){
    int r;
    int fd;
    ...
    uint32_t byte_size  = _nb_regs*4;   // 32 bits axi lite only (for now)
    ...
    if(fd != -1){
        // Map physical address into user space by getting a virtual address
        _axi_regs_u32_vptr = (uint32_t *)mmap(NULL, byte_size, PROT_READ|PROT_WRITE, MAP_SHARED, fd, offset);
        r = (_axi_regs_u32_vptr == NULL) ? EXIT_FAILURE : EXIT_SUCCESS;
        statusPrint(r, "Map pointer to virtual address of axi lite");
