...
int AxiDma::monitoring(struct sw_config swconfig){
	...
	string fpath_save_spk	= swconfig.save_path + "raster_" +  fname + ".csv";
	string fpath_save_vmem	= swconfig.save_path + "waves_"  +  fname + ".csv";
	string fpath_save_stim	= swconfig.save_path + "stim_"   +  fname + ".csv";
	...
	rx_th_args[TH_ID_SPK_MON].transfer_size_bytes	= (DATAWIDTH_TIME_STEP+MAX_NB_NRN*MAX_NB_SEG)/8*swconfig.nb_tstamp_per_spk_transfer;
	...
	rx_th_args[TH_ID_SPK_MON].send					= swconfig.en_zmq_spikes;
	rx_th_args[TH_ID_SPK_MON].save					= swconfig.save_local_spikes;
	...
	rx_th_args[TH_ID_VMEM_MON].send					= swconfig.en_zmq_vmem;
	rx_th_args[TH_ID_VMEM_MON].save					= swconfig.save_local_vmem;
	...
	tx_th_args[TH_ID_EXT_STIM].transfer_size_bytes	= (MAX_NB_NRN*MAX_NB_SEG*DATAWIDTH_DMA_STIM)/8;
	tx_th_args[TH_ID_EXT_STIM].nb_transfer			= -1;
	tx_th_args[TH_ID_EXT_STIM].send					= false;
	tx_th_args[TH_ID_EXT_STIM].save					= false;
...
void AxiDma::rxThreadSpikes(void* args){
	...
	for (int i = 0; i < nb_read_dma; i++){
		...
		if(save){
			...
			vector<uint32_t> x; // static allocation might get better performance
			vector<uint32_t> y;
			...
			// For each frame
			const unsigned int frame_size = (DATAWIDTH_TIME_STEP+MAX_NB_NRN*MAX_NB_SEG)/(8*sizeof(unsigned int)); // TODO : make generic for frame size (here 17 for 512 neurons + 1 tstamp)
			for (int j = 0; j < ((transfer_size_bytes/sizeof(unsigned int))/frame_size); j++){ 
				tstamp = buffer[frame_size*j];
				// For all spike registers
				for (int k = 0; k < (MAX_NB_NRN*MAX_NB_SEG)/32; k++){
