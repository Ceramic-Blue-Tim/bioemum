...
int HwCfg::apply(){
    ...
    r = applyGeoparam();
    statusPrint(r, "Apply geometric parameters configuration to hardware");
    if (r == EXIT_FAILURE)
        return r;
...
int HwCfg::applyHhparam(){
    ...
    // Apply hhparams
    for (int waddr = 0; waddr < _nb_nrn*_nb_seg; waddr++){
        ...
        for (int p = 0; p < _nb_hhparam; p++){
            if(p == HHP_ID_NOISE_OFFS){
                _axilite->write(stof(HHp_Ni[p]), REGW_HHPARAM_BASE+p, SFI_MU_NOISE_INT, SFI_MU_NOISE_DEC);
            }
            else if(p == HHP_ID_PMUL_THETA){
                _axilite->write(stof(HHp_Ni[p]), REGW_HHPARAM_BASE+p, SFI_THETA_NOISE_INT, SFI_THETA_NOISE_DEC);
            }
            else if(p == HHP_ID_PMUL_SIGMA){
                _axilite->write(stof(HHp_Ni[p]), REGW_HHPARAM_BASE+p, SFI_SIGMA_NOISE_INT, SFI_SIGMA_NOISE_DEC);
            }
                ...
                if (p==HHP_ID_V_INIT){
                    _axilite->write(stof(HHp_Ni[p]), REGW_V_INIT_SYN_SFI, SFI_V_SYN_INT, SFI_V_SYN_DEC);
                }
            }
        }
 ...
 * Apply geometric parameters
 * 
 *          geoparam (D_const, nrn_area, U, p)
 *  N0S0    0.1,0.2,0.3,0.4
 *  N0S1    0.1,0.2,0.3,0.4
 * 
 * @return EXIT_SUCCESS
****************************************************************************/
int HwCfg::applyGeoparam(){
    string line;
    vector<string> vline;   // [0] (string) key [1] (string) val
    vector<string> Geop_Ni;
    ...
    for (int waddr = 0; waddr < _nb_nrn*_nb_seg; waddr++){
        ...
        getline(_hw_cfg_file, line);
        vline   = splitStr(line, KEY_SEP);
        Geop_Ni  = splitStr(vline[1], VAL_SEP);
        
        // Fill AXI registers
        float D_const_fp    = stof(Geop_Ni[0]);
        float nrn_area_fp   = stof(Geop_Ni[1]);
        float U_fp          = stof(Geop_Ni[2]);
        uint32_t p_int      = stoi(Geop_Ni[3]);
        
        uint32_t D_const_fp_int;    memcpy(&D_const_fp_int,   &D_const_fp,  sizeof(D_const_fp_int));
        uint32_t nrn_area_fp_int;   memcpy(&nrn_area_fp_int,  &nrn_area_fp, sizeof(nrn_area_fp_int));
        uint32_t U_fp_int;          memcpy(&U_fp_int,         &U_fp,        sizeof(U_fp_int));
        
        _axilite->write(D_const_fp_int,     REGW_D_CONST);
        _axilite->write(nrn_area_fp_int,    REGW_NRN_AREA);
        _axilite->write(U_fp_int,           REGW_U);
        _axilite->write(p_int,              REGW_PTREE);

        // Write address and enable writing
        _axilite->write(waddr, REGW_WADDR_GEOPARAM);
        _axilite->write((1<<0), REGW_WEN_GEOPARAM);

        // Wait for FPGA read and disable writing
        while(_axilite->read(REGR_WADDR_GEOPARAM_LP) != waddr);
        _axilite->write((0<<0), REGW_WEN_GEOPARAM);
...
int HwCfg::applySynrates(){
    ...
    for (int waddr = 0; waddr < _depth_synrate; waddr++){
        ...
        // Fill AXI registers
        _axilite->write(stof(rates[0]), REGW_SYNRATE_BV, SFI_SYNRATE_BV_INT,            SFI_SYNRATE_BV_DEC); // Bv
        _axilite->write(stof(rates[1]), REGW_SYNRATE_TV, SFI_SYNRATE_TV_INT,            SFI_SYNRATE_TV_DEC); // Tv
        _axilite->write(stof(rates[2]), REGW_SN_GABAB,   SFI_SYNRATE_SN_GABAB_OUT_INT,  SFI_SYNRATE_SN_GABAB_OUT_DEC); // sn_gabab
...
int HwCfg::applySynParams(){
    ...
    for (uint8_t wsel = 0; wsel < NB_PSYN; wsel++){
        ...
        if( wsel==PSYN_ID_AMPA_K1 || wsel==PSYN_ID_NMDA_K1 || wsel==PSYN_ID_GABAA_K1 || wsel==PSYN_ID_GABAB_K1 ){
            ...
            fp_int = SFI_I_SYN_INT;
            fp_dec = SFI_I_SYN_DEC;
        }
...
int HwCfg::applySynConf(){
    ...
    // Synaptic type
    for (int dest = 0; dest < _nb_nrn*_nb_seg; dest++){
        // Read line and split key/value
        getline(_hw_cfg_file, line);
        vline       = splitStr(line, KEY_SEP);
        twsyn_line  = splitStr(vline[1], VAL_SEP); // All synaptic types of neuron nid

        for (int src = 0; src < _nb_nrn*_nb_seg; src++){
            ...
            twsyn[cnt_col_offs*3 + cnt_col + (offs_line+offs_rest)*(NB_RAM_TWSYN*RAMWIDTH_TWSYN/16) + offs_depth*NB_DSP_WMB_SYNAPSES*(MAX_NB_NRN*MAX_NB_SEG-1)] = ((uint16_t)(tsyn_i)<<WEIGHT_SIZE) | (float2sfi(stof(str_wsyn_i), SFI_WSYN_INT, SFI_WSYN_DEC)&((1<<WEIGHT_SIZE)-1));
    ...
    #ifdef DBG_PROBE_TWSYN_EXPORT
        ofstream flist[NB_RAM_TWSYN];
        for (int i = 0; i < NB_RAM_TWSYN; i++){
            flist[i].open("./uram"+to_string(i)+".txt");
        }
    #endif
    ...
    for (int waddr = 0; waddr < RAMDEPTH_TWSYN; waddr++){
        for (int ram_i = 0; ram_i < NB_RAM_TWSYN; ram_i++){
            ...
            #ifdef DBG_PROBE_TWSYN_EXPORT
                flist[ram_i] << bitset<8>((uint8_t)twsyn_vptr_u32_reg2) <<  bitset<32>(twsyn_vptr_u32_reg1) << bitset<32>(twsyn_vptr_u32_reg0) << endl;
            #endif
