...
class HwConfigFile :
    def __init__(self, sw_ver, NB_NEURONS, NB_SEGMENTS):
        ...
        self.nb_seg     = NB_SEGMENTS

        # HH parameters
        self.nb_hhparam  = 0
        self.nb_geoparam = 0
        self.HH_param   = []    # Hodgkin-Huxley model parameters (hhparam; nrn)
        self.geo_param  = []    # Hodgkin-Huxley model parameters (hhparam; nrn)
        ...
        with open(fpath, "w") as f:
            ...
            for nrn in range(self.nb_nrn):
                for seg in range(self.nb_seg):
                    f.write("hhparam_N{}S{}".format(nrn, seg) + KEY_SEP + VAL_SEP.join(map(self.__formatFloat, self.HH_param[nrn*self.nb_seg + seg][:])) + '\n')

            for nrn in range(self.nb_nrn):
                for seg in range(self.nb_seg):
                    f.write("geoparam_N{}S{}".format(nrn, seg) + KEY_SEP + VAL_SEP.join(map(str, self.geo_param[nrn*self.nb_seg + seg][:])) + '\n')

            # Synapses parameters
            f.write("psyn" + KEY_SEP + VAL_SEP.join(map(self.__formatFloat, self.psyn)) + '\n')
            ...
            for nrn in range(self.nb_nrn):
                for seg in range(self.nb_seg):
                    f.write("N{}S{}".format(nrn, seg) + KEY_SEP + VAL_SEP.join(str(tsyn) + '$'+ str(wsyn) for tsyn,wsyn in zip(self.tsyn[nrn*self.nb_seg + seg][:], self.wsyn[nrn*self.nb_seg + seg][:])) + '\n')
