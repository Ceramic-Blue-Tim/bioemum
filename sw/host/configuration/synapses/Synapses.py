...
# > **04 Sep 2024** : fork from BioemuS adapted to BioemuM equations (RB)
...
class Destexhe:
    ...
    # Synaptic currents #########################################
    def getPsyn(self, dt):
        ret_psyn = list(self.psyn)
        ret_psyn[self.PID["GABAb_K3"]] = ret_psyn[self.PID["GABAb_K3"]] * dt
        ret_psyn[self.PID["GABAb_K4"]] = ret_psyn[self.PID["GABAb_K4"]] * dt
        return ret_psyn
    ...
    def calcISynAmpa(self, vpre, vpost, rprev, wsyn, dt):
        ...
        gnew_syn  = wsyn * gsyn * rnew
        gnewe_syn = gnew_syn * Esyn
        return [gnew_syn, gnewe_syn, rnew]
    ...
    def calcISynNmda(self, vpre, vpost, rprev, wsyn, dt):
        ...
        rnew      = self.rcalc(rprev, A, B, self.T_v(vpre), dt)
        gnew_syn  = wsyn * gsyn * self.B_v(vpost) * rnew
        gnewe_syn = gnew_syn * Esyn
        return [gnew_syn, gnewe_syn, rnew]
    ...
    def calcISynGabab(self, vpre, vpost, sprev, rprev, wsyn, dt):
        ...
        gnew_syn  = wsyn * gsyn * (snew**n)/(snew**n + Kd)
        gnewe_syn = gnew_syn * Esyn
        return [gnew_syn, gnewe_syn, snew, rnew]